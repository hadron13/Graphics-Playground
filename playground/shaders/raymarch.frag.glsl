...
float map(vec3 position){
    float dist = sphere(vec3(-0.3, 0, 0.5), 0.25, position);
    dist = join (dist, sphere(vec3(0.3, 0, 0.5), 0.25, position));
    return dist;
}
...
void main(){
    ...
    vec3 ray_position = vec3(0, 0, -0.4);
    ...
    for(int i = 0; i < 100; i++){
        ...
        if(dist < 0.00001){
            ...
            vec3 light_position = vec3(2.0, 1.0, 0.5);
            ...
            float attenuation = 1.0 / (0.1 + 0.5 * light_distance + 0.05 * light_distance * light_distance); 
            
            vec3 ambient = 0.1 * light_color;
            vec3 diffuse = (max(dot(normal, light_direction), 0)) * light_color;
            vec3 specular = pow(max(dot(-ray_direction, reflect_direction), 0.0), 16) * 0.5 * light_color;

            float shadow = shadow(ray_position, light_direction, 0.01, light_distance);

            color = (ambient + diffuse + specular) * attenuation * obj_color * shadow;
